...
Load in data
```{r,error=FALSE,warning=FALSE}
cf = fread('./outputs/gapfilling/churchill_2022_2025_era.csv')
...
rf.swc = randomForest(formula = swc ~ tair + rh + rg + ws + tsoil + vpd + pres + le + h,data = train,ntree = 20, importance = TRUE)
...
rfnee = randomForest(formula = nee ~ tair + rh + rg + ws + tsoil + swc + vpd + le + pres + h, data = train, ntree = 20, importance = TRUE)
...
rfch4 = randomForest(formula = fch4 ~ tair + rh + rg + ws + tsoil + vpd + swc + h + le,data = train,ntree = 20,importance = TRUE)
