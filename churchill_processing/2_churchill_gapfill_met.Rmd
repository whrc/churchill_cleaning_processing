...
Load Data
These files are found on the buckets under /cleaned
```{r}
c22 = fread(input = "./data/cleaned/Churchill_Met_Flux_Clean_Ameriflux_2022.csv")
c23 = fread(input = "./data/cleaned/Churchill_Met_Flux_Clean_Ameriflux_2023.csv")
c24 = fread(input = "./data/cleaned/Churchill_Met_Flux_Clean_Ameriflux_2024.csv")
c25 = fread(input = "./data/cleaned/Churchill_Met_Flux_Clean_Ameriflux_2025.csv")


df <- rbind(c22,c23,c24,c25,fill=T)

# Adjust timestamps to CST

## Fix data set timestamp adjustment here- MOVE TS ADJUSTMENT TO FILE MERGING

df$ts <- df$ts-6*3600 #adjust timezone from UTC to CST
df$TIMESTAMP <- df$TIMESTAMP-6*3600 #adjust timezone from UTC to CST

# create date, time, DOY in the correct  time zone
df$DOY <- yday(df$TIMESTAMP)
df$time <- format(df$TIMESTAMP, format = "%H:%M")
...
```{r}
summary(df$RH_19_3_1_1_1.c) #check on the data coverage after filling small gaps
plot(df$ts,df$RH_19_3_1_1_1.c)

#save off a copy of the newly merged data
write.csv(x = df,file = "./outputs/gapfilling/churchill_2022_2025.csv",row.names = F)
```

Add ERA5 data
```{r}
#load in era5 data for churchill for continuous met data
era = fread('./outputs/gapfilling/era5_churchill25.csv')
...
#why is linear regression less than both datasets here?
```
...
#Problem days:
#2025-03-03
#2025-06-28 (known clock change in change log)
# add cleaning code check in file merging code instead of here

ggplot(data = all)+
  geom_point(aes(date,rad,col='ERA5'))+
  geom_point(aes(date,SWIN_6_10_1_1_1.c,col='Site'))+
  scale_x_datetime(limits = as.POSIXct(c('2025-02-23','2025-03-05')))
...
```{r}
eras = all[,c('date','rh.eramod','airt.eramod','ws.eramod','rad.eramod','tsoil.eramod','le.eramod','h.eramod','pres.eramod','dew','st1','st2','le','pres','h','rad','airt','ppt','u','v','vwc1','vwc2')]

df = df[!duplicated(df$date),]
df = merge(df,eras,by = 'date',all = T)

write.csv(x = df,file = './outputs/gapfilling/churchill_2022_2025_era.csv',row.names = F,quote = F)

#calculate VPD from air t and RH
svp = 610.7*10^((7.5*all$airt)/(237.3+all$airt))
all$vpd = ((100 - all$rh)/100)*svp  
```


Plot comparing churchill ERA data to site level data
```{r}
#air T
airt = ggplot(data = all,aes(airt,TA_2_1_1_1_1))+theme_bw()+
...
#RH
rh = ggplot(data = all,aes(rh,RH_19_3_1_1_1.c))+theme_bw()+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
...
#wind speed
ws = ggplot(data = all,aes(ws,WS_16_33_1_1_1))+theme_bw()+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
  scale_y_continuous(expression('Site Wind Speed ('*m~s^-1*")"))+
  scale_x_continuous(expression('ERA5 Wind Speed ('*m~s^-1*")"))+
  theme(text = element_text(size = 6))

#'rad.eramod'
rad = ggplot(data = all,aes(rad,SWIN_6_10_1_1_1.c))+theme_bw()+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
  scale_y_continuous(expression('Site Incident Radiation ('*W~m^-2*")"))+
  scale_x_continuous(expression('ERA5 Incident Radiation ('*W~m^-2*")"))+
  theme(text = element_text(size = 6))

#'tsoil.eramod',
tsoil = ggplot(data = all,aes(st2,TS_2_38_3_2_1.c))+theme_bw()+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
...
le = ggplot(data = all,aes(le,LE.c))+theme_bw()+
  ...
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
  scale_y_continuous(expression('Site LE ('*W~m^-2*")"))+
  scale_x_continuous(expression('ERA5 LE ('*W~m^-2*")"))+
  theme(text = element_text(size = 6))
...
h = ggplot(data = all,aes(h,H.c))+theme_bw()+
  ...
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
  scale_y_continuous(expression('Site H ('*W~m^-2*")"))+
  scale_x_continuous(expression('ERA5 H ('*W~m^-2*")"))+
  theme(text = element_text(size = 6))


#VPD
vpd = ggplot(data = all,aes(vpd,VPD))+theme_bw()+
#  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
  scale_y_continuous(expression('Site VPD ('*W~m^-2*")"))+
  scale_x_continuous(expression('ERA5 VPD ('*W~m^-2*")"))+
  theme(text = element_text(size = 6))

#swc
swc = ggplot(data = all,aes(vwc1*100,SWC_12_36_2_1_1))+theme_bw()+
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
...
```{r}
png(filename = './png/era5_churchill_comparisons.png',width = 6,height = 4,units = 'in',res = 1500)
plot_grid(airt,rh,ws,rad,tsoil,le,h,vpd,swc)
dev.off()
```
