...
##Pressure
```{r}
all$pres = all$pres/1000 # adjust units
...
#pres
pres = ggplot(data = all,aes(pres,PA_4_2_1_1_1.c))+theme_bw()+
  geom_point(alpha=0.5)+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)+
  scale_y_continuous(expression('Site Pressure (kPa)'))+
  scale_x_continuous(expression('ERA5 Pressure (kPa)'))+
  theme(text = element_text(size = 10))
```

```{r}
png(filename = './png/era5_churchill_comparisons_sz10.png',width = 10,height = 8,units = 'in',res = 1500)
plot_grid(airt,rh,ws,rad,tsoil,le,h,vpd,pres)
dev.off()
```
