---
title: "Neural Network Churchill"
output:
...
Load in data
```{r,error=FALSE,warning=FALSE}
cf = fread('./churchill_2022_2024_era.csv')
...
rf.swc = randomForest(formula = swc ~ tair + rh + rg + ws + tsoil + vpd + pres + le + h,data = train,ntree = 100, importance = TRUE)
...
# boruta = Boruta(nee ~ tair + rh + rg + ws + tsoil + swc + pres + vpd + le + h ,data = cc,doTrace = 2,maxRuns = 100)
# 
# plot(boruta,las = 2)
```
...
rfnee = randomForest(formula = nee ~ tair + rh + rg + ws + tsoil + swc + vpd + le + pres + h, data = train, ntree = 100, importance = TRUE)
...
ggplot(data = cf)+theme_bw()+
  ...
  scale_x_datetime(limits = as.POSIXct(c("2024-03-01","2024-03-30")))
...
ggplot(data = cf)+theme_bw()+
  ...
  scale_x_datetime(limits = as.POSIXct(c("2024-06-01","2024-06-30")))
...
# png("C:/Users/dtrangmoe/Documents/Churchill/rf_validations.png",width = 5,height = 3,units = 'in',res = 1500)
...
save off the data
```{r,error=FALSE,warning=FALSE}
write.csv(x = cf,file = "./churchill_2022_2024_gf.csv",quote = F,row.names = F)
```
