...
Load in data
```{r,error=FALSE,warning=FALSE}
cf = fread('./churchill_2022_2025_era.csv')
...
ggplot(data = cf)+theme_bw()+
  ...
  scale_x_datetime(limits = as.POSIXct(c("2025-03-01","2025-03-30")))

ggplot(data = cf)+theme_bw()+
  geom_point(aes(date,rfnee,col='RF'),alpha=0.2)+
  geom_point(aes(date,co2_flux.c,col='Real'),alpha=0.2)+
  scale_x_datetime(limits = as.POSIXct(c("2025-06-01","2025-07-30")))
```
...
png("./rf_validations.png",width = 5,height = 3,units = 'in',res = 1500)
co2.rf + ch4.rf
dev.off()
```
...
save off the data
```{r,error=FALSE,warning=FALSE}
write.csv(x = cf,file = "./churchill_2022_2025_gf.csv",quote = F,row.names = F)
```
