...
write.csv(df,'./outputs/cleaning/Churchill_fluxes_merged_2025.csv',row.names = F)
...
fp = './data/biomet/'
files = list.files(path = fp,pattern = 'churchill_biomet_2025[0-9][0-9].+csv$',recursive = T,full.names = T)
...
write.csv(x = df,file = './outputs/cleaning/Churchill_met_merged_2025.csv',row.names = F,quote = F)
...
Add a few more plots for checks
change other sites to rbindfill, copy use.names = T
