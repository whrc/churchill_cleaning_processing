...
```{r, warning=FALSE}
flux_dates <- c('2022-08-01', '2024-09-15')
```
...
```{r}
signal.threshold.co2 = ifelse(df$co2_signal_strength_7200_mean >= 75 |
                              df$co2_signal_strength_7500_mean >= 75,0,1)
...
Tau.limits = df$Tau
Tau.limits = ifelse(df$Tau < -1 | df$Tau > .1,1,0)

ggplot(data = df)+
  geom_point(aes(x = TIMESTAMP, y = Tau, color = Tau.limits))
...
ggplot(data = df)+
  ...
  scale_y_continuous(limits = c(-20,20))+
scale_x_datetime(limits = as.POSIXct(c('2022-08-03','2022-08-30'),format="%F"))
```
