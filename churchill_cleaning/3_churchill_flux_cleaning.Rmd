...
df = fread('C:/Users/dtrangmoe/Documents/Churchill/Churchill_fluxes_merged_2024.csv')
...
```{r, warning=FALSE}
flux_dates <- c('2024-01-01', '2024-12-31')
```
...
Tau.limits = df$Tau
Tau.limits = ifelse(df$Tau < -1.1 | df$Tau > .1,1,0)
...
co2_flux.limits = ifelse(df$co2_flux > 30 | df$co2_flux < -30,1,0)
```
...
ch4_flux.limits =ifelse(df$ch4_flux < -0.5 | df$ch4_flux > 1,1,0)
```
...
ggplot(data = df)+
  geom_point(aes(ts,H))+
  scale_y_continuous(limits = c(-200,500))+
  geom_hline(yintercept = c(-200,400))

H.limits =ifelse(df$H < -200 | df$H > 400,1,0)
```
...
ggplot(data = df)+
  ...
  scale_y_continuous(limits = c(-200,500))+
  geom_hline(yintercept = c(-100,500))

LE.limits =ifelse(df$LE < -100 | df$LE > 500,1,0)
```
...
ggplot(data = df)+
  ...
  scale_y_continuous(limits = c(-20,20))
#+scale_x_datetime(limits = as.POSIXct(c('2024-01-03','2024-02-30'),format="%F"))
```
...
write.csv(x = df,file = 'C:/Users/dtrangmoe/Documents/Churchill/Churchill_Met_Flux_Clean_Ameriflux_2024.csv',row.names = F,quote = F)
```
