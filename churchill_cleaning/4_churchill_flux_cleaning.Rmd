...
```{r, warning=FALSE}
flux_dates <- c('2025-01-01', '2025-09-30')
```
...
ggplot(data = df)+
  ...
  geom_hline(yintercept = c(-15,8)) ## expand absolute limits here to do less cleaning

#This filter removes points outside of absolute limits and high uptake, low light points
co2_flux.limits <- df$co2_flux
co2_flux.limits = replace(co2_flux.limits, (df$co2_flux > 8 | df$co2_flux < -15 | (df$co2_flux < -5 & df$PPFD_7_21_1_1_1.c < 150)),NA)
...
ggplot(data = df)+
  ...
  geom_hline(yintercept = c(-0.15,0.28)) ## expand absolute limits here to do less cleaning

ch4_flux.limits <- df$ch4_flux
ch4_flux.limits = replace(ch4_flux.limits, (df$ch4_flux < -0.15 | df$ch4_flux > 0.28),NA)
...
ggplot(data = df)+
  ...
  scale_y_continuous(limits = c(-200,500))+
  geom_hline(yintercept = c(-200,300)) ## expand absolute limits here to do less cleaning

H.limits <- df$H
H.limits=replace(H.limits, df$H < -200 | df$H > 300, NA)
...
ggplot(data = df)+geom_hline(yintercept = 0)+
  ...
  scale_x_datetime(limits = as.POSIXct(c("2025-07-14","2025-07-20")))
```
